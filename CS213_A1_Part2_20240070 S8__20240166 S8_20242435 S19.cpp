...
/* 
Adham Amr Hassan (20240070): filter number = 2, 5, 8, 11
Hussien mahmoud (20240166) : filter number = 2, 5, 8, 11
Adham Ayman Ahmed (20242435): filter number = 3,6,9,12
*/
...
Image applyBlur(Image &img) {
    ...
    int k = 2; // matrix for area of pixels (5 X 5)
    int count = ( 2 * k + 1) * ( 2 * k + 1); //get the total number of pixels
    for (int x = k; x < img.width - k; x++) {
        for (int y = k; y < img.height - k; y++) {
            ...
            for (int c = 0; c < 3; c++) {
                temp(x, y, c) = sum[c] / count; // we take the pixel itself, plus the 8 surrounding pixels.
            }
        }
    }
...
int main() {
        ...
        cout << " VERY IMPORTANT: Please make sure image has same directory as project file to be able to find the photo" << endl ; //Very important
        cout << " \n ---------------------------------------------------------------------------------------------------\n";
    ...
    cout << "Please enter the image you want to edit :)\n";
        cin >> filename;
    cout << " ##congratulations, the file is uploaded sucessfuly##\n";
    ...
    Image image(filename);
        ...
        {
            int strength;
            cout << "Enter blur strength ( 1 = light, 2 = medium, 3 = strong): ";
            cin >> strength;
